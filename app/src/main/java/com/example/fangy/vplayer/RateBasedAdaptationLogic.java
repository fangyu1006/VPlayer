...
public class RateBasedAdaptationLogic implements AdaptationLogic {

    private static final String TAG = RateBasedAdaptationLogic.class.getSimpleName();
    ...
    public RateBasedAdaptationLogic() {
    ...
    private Representation calculateRepresentation(AdaptationSet adaptationSet) {
        ...
        if(state.currentRepresentation == null) {
             ...
             * If the there have been consecutive upvotes for at least 6 seconds in a row, the
            ...
            if(state.vote < 0 || state.vote >= Math.max(1, 6000000d / state.currentRepresentation.segmentDurationUs)) {
